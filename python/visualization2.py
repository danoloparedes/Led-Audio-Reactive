...
p2 = np.tile(1.0, (3, config.N_PIXELS))
...
def visualize_energy(y):
    ...
    p[0, :] = gaussian_filter1d(p[0, :], sigma=10.0)
    p[1, :] = gaussian_filter1d(p[1, :], sigma=10.0)
    p[2, :] = gaussian_filter1d(p[2, :], sigma=10.0)
...
def visualize_fade(y):
    global contador_fade
    contador_fade=contador_fade+1
    if contador_fade >255: 
        contador_fade=contador_fade-255

    r,g,b = wheel(contador_fade)
...
def visualize_arcoiris(y):
    global contador_arcoiris
    global p2
    contador_arcoiris = contador_arcoiris + 1
    if contador_arcoiris >255:
        contador_arcoiris=contador_arcoiris-255
    for i in range(config.N_PIXELS // 2):
        pixel_index = (i * 256 // config.N_PIXELS) + contador_arcoiris*3
...
def boton4fade():
    global visualization_effect
    visualization_effect = visualize_fade
def boton5arcoiris():
    global visualization_effect
    visualization_effect = visualize_arcoiris


def estadoluces(modo):
    # print('prayge')
    # print(modo)
    estado = modo['estado']  # Accede al valor asociado con la clave 'estado'
    if estado == "Scroll":
        boton1scroll()
    elif estado == "Energy":
        boton2energy()
    elif estado == "Spectrum":
        boton3spectrum()
    elif estado == "Fade":
        boton4fade()
    elif estado == "Rainbow":
...
# boton1.when_pressed = boton1scroll
# boton2.when_pressed = boton2energy
# boton3.when_pressed = boton3spectrum
# boton4.when_pressed = boton4fade
# boton5.when_pressed = boton5arcoiris
...
if __name__ == '__main__':
    if config.USE_GUI:
        ...
        led_plot.setRange(yRange=[-5, 260]) #-5,260 original
    ...
    list_threads = []
    thread_microphone = threading.Thread(target=microphone.start_stream, args=(microphone_update,))
    thread_microphone.daemon = True
    thread_check_file = threading.Thread(target=check_file)
    thread_check_file.daemon = True

    # Inicia los hilos
    for thread in [thread_microphone, thread_check_file]:
        thread.start()
